...
- タスク状態は `- [ ]`（ユーザーが未承認・未完了）と `- [x]`（ユーザーが承認・完了とした項目）だけです。AIが仮作成しただけではチェックを入れません。
- 仮作成した成果物へのリンクは、チェックOFFのまま各項目に掲載します。ユーザーがその項目を承認・完了としたらチェックONにします。
- 一括仮作成は成果物一式の保存・提示まで進めます。再開時はチェック状態ではなく、リンク先の成果物と内容を確認して重複作成を避けます。
...
「事業を決める」で内容を記録したら、AIが短い英小文字の名前を決め、例えば `ventures/counseling/` に一度変更します。一括仮作成では仮設定の記録時に行い、チェックONを待ちません。同名の別事業がある場合は `counseling-002` から未使用の番号を付けます。タスク・成果物・チェック状態を保持し、参照リンクも更新します。屋号の決定ではフォルダ名を変更しません。詳しい手順は [AGENTS.md](AGENTS.md#事業フォルダの命名と変更) に記載しています。
